<template>
  <div class="delete-user-container">
    <h2>{{ user.name }}</h2>
    <p>Are you sure want to reset password for "{{ user.email }}" user?</p>
...
export default {
  ...
  methods: {
    async resetPassword () {
      await this.$axios.post('/reset_password/', {'email': this.user.email}).then((res) => {
        this.$emit('close-and-refresh', res.data.data, 'success')
      }).catch((e) => {
